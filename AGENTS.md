...
<skill>
<name>react-doctor</name>
<description>Run after making React changes to catch issues early. Use when reviewing code, finishing a feature, or fixing bugs in a React project.</description>
<location>global</location>
</skill>
...
- **No Redundant Build Checks**: For small CSS, content, or styling changes, there is no need to run a full `npm run build` check every time. Only compile the production bundle when explicitly requested, during major architectural changes, or upon final verification.
