...
const ExperienceItem = ({
	...
	return (
		<div
			className={`group border-b border-border py-8 transition-colors duration-500 cursor-pointer ${
		...
		>
			<div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6 items-center">
				...
				<div className="md:col-span-3 flex items-center gap-3 font-mono">
					<span className="text-[10px] text-zinc-400 dark:text-zinc-600 font-medium select-none">
						{displayIndex}
					</span>
					<span className="text-[11px] font-semibold tracking-wider text-zinc-500 group-hover:text-zinc-700 dark:group-hover:text-zinc-300 transition-colors uppercase">
						{experience.period}
					</span>
				</div>
				...
				<div className="md:col-span-8 flex items-center gap-4">
					<div className="flex flex-col gap-0.5">
						<h3 className="font-display font-bold text-foreground/90 group-hover:text-foreground transition-colors text-base md:text-lg flex items-center gap-3">
							{experience.company}
							{experience.status && (
								<span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[9px] font-semibold border border-emerald-500/20 font-mono tracking-wider uppercase">
				...
				<div className="md:col-span-1 hidden md:flex justify-end">
					<div
						className={`p-1.5 rounded-full border border-border text-muted-foreground transition-all duration-500 bg-muted/20 ${
							isExpanded
								? "bg-foreground text-background rotate-180 border-transparent shadow-[0_0_15px_rgba(0,0,0,0.05)] dark:shadow-[0_0_15px_rgba(255,255,255,0.1)]"
								: "group-hover:text-foreground group-hover:border-border-hover"
						}`}
					>
			...
			>
				<div className="overflow-hidden">
					<div className="grid grid-cols-1 lg:grid-cols-12 gap-8 pt-6 border-t border-border text-left items-start">
						{/* Left Column: Asymmetric monospaced bullet achievements (Col 8) */}
						<div className="lg:col-span-8 flex flex-col gap-4">
							<span className="text-[9px] font-mono tracking-[0.2em] text-muted-foreground uppercase block mb-1">
								// Achievements & Impact
							</span>
							<ul className="space-y-3.5">
								{experience.description &&
									experience.description.map((point, i) => (
										<li
											key={i}
											className="text-xs md:text-sm leading-relaxed text-muted-foreground flex items-start gap-4"
										>
											<span className="text-muted-foreground/60 font-mono text-[10px] pt-0.5 select-none font-bold">
						...
						<div className="lg:col-span-4 flex flex-col gap-6 lg:border-l lg:border-border lg:pl-6">
							{/* Tech Stack */}
									...
									<div className="flex items-center gap-2">
										{experience.links.map((link, i) => (
											<a
												...
												className="w-8 h-8 rounded-lg flex items-center justify-center text-muted-foreground hover:text-foreground bg-muted/50 border border-border hover:border-border-hover transition-all group/link"
